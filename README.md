...
- **Админ‑панель**
  - `/admin` меню (добавить товар, заказы).
...
**Админ:**
- `/admin` — главное меню («➕ Добавить товар», «📦 Заказы»).
- Добавление товара (FSM): `title → price → category_id → description → photos(loop) → active → confirm`  
...
- Никогда не коммитьте реальный `BOT_TOKEN`/пароли. Храните их в `.env` и секретах CI/CD.
- Для тестов используйте отдельную БД/учётку.
- Логи могут содержать служебную информацию — не храните их в репозитории.
...
- [ ] FSM **редактирования товара** (`/admin_edit_product <id>`): поля + управление галереей (добавить/удалить/переупорядочить).
- [ ] Улучшение UX карточки: галерея с пагинацией по кнопкам, предпросмотр цен/остатков.
- [ ] Фильтры/поиск по товарам.
- [ ] Промокоды/купоны, варианты доставки и оплаты.
- [ ] Тесты уровня сервисов/репозиториев (моки), интеграционные тесты.
...
Вопросы и предложения — в Issues. Для ускорения диагностики прикладывайте логи (`logs/`) и точные шаги воспроизведения.
