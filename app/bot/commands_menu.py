"""
Меню команд Telegram (кнопка слева внизу), вынесено в отдельный модуль.

Правильная стратегия:
- При запуске выставляем ТОЛЬКО общие команды для всех пользователей.
- Персональные админ-команды добавляем точечно для чата админа,
  когда он напишет боту (например, в хэндлере /start).

Так избегаем ошибки Telegram "Bad Request: chat not found" для админов,
которые ещё не открывали диалог с ботом.
"""
